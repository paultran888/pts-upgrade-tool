...
// Disable the SDK's pre-flight timeout check — it incorrectly throws for streaming
// requests with Opus + high max_tokens, calculating the operation would exceed 10 minutes
// and throwing BEFORE the stream:true parameter is processed.
if (typeof client._calculateNonstreamingTimeout === 'function') {
  client._calculateNonstreamingTimeout = () => {};
}
...
The business owner will see this as a before/after comparison against their current site. It MUST be dramatically better at first glance while still feeling like THEIR brand — evolved, not replaced.`;
  ...
  // Use streaming with extended timeout to handle Opus's longer generation time
  // The SDK's _calculateNonstreamingTimeout throws before checking stream:true,
  // so we must explicitly set a long timeout to bypass the pre-flight check
  let html = '';
  const stream = await client.messages.create({
    ...
    timeout: 30 * 60 * 1000  // 30 minutes — streaming chunks arrive continuously so no real hang risk
  });
