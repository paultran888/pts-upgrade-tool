...
async function auditWebsite(url) {
  ...
  try {
    ...
    if (isHttps) {
      totalPoints += 10;
      findings.push({ category: 'Security', label: 'HTTPS enabled', pass: true, points: 10, maxPoints: 10, detail: 'Your site uses a secure SSL connection.' });
    } else {
      findings.push({ category: 'Security', label: 'No HTTPS', pass: false, points: 0, maxPoints: 10, detail: 'Your site isn\'t using HTTPS. Visitors see a "Not Secure" warning, and Google penalizes your search ranking.' });
    }
    ...
    if (loadTimeMs < 2000) {
      speedPoints = 15;
      speedDetail = `Your site loaded in ${(loadTimeMs / 1000).toFixed(1)}s — that's fast.`;
      ...
      speedPoints = 5;
      speedDetail = `Your site took ${(loadTimeMs / 1000).toFixed(1)}s to load. 53% of visitors leave if a page takes longer than 3 seconds.`;
    } else {
      speedPoints = 0;
      speedDetail = `Your site took ${(loadTimeMs / 1000).toFixed(1)}s to load. That's slow enough to lose most visitors before they even see your content.`;
    }
    ...
    if (hasMobileViewport) {
      ...
      findings.push({ category: 'Mobile', label: 'No mobile viewport', pass: false, points: 0, maxPoints: 10, detail: 'Your site is missing a responsive viewport meta tag. On phones, it will look like a shrunken desktop page — hard to read and navigate.' });
    }
    ...
    if (pageData.metaDesc && pageData.metaDesc.length >= 50 && pageData.metaDesc.length <= 160) {
      ...
      findings.push({ category: 'SEO', label: 'Missing meta description', pass: false, points: 0, maxPoints: 5, detail: 'No meta description found. Google will auto-generate one from your page content, which rarely looks good.' });
    }
    ...
    if (ogCount === 3) {
      ...
      findings.push({ category: 'SEO', label: 'No social sharing tags', pass: false, points: 0, maxPoints: 5, detail: 'No Open Graph tags found. When someone shares your site on Facebook, LinkedIn, or Twitter, it will look like a plain link with no preview.' });
    }
    ...
    if (pageData.ctaCount >= 2) {
      ...
      findings.push({ category: 'Conversion', label: 'No clear call-to-action', pass: false, points: 0, maxPoints: 10, detail: 'No obvious CTA buttons found. Visitors don\'t know what action to take — call, book, order? You\'re losing potential customers.' });
    }
    ...
    findings.push({
      ...
      detail: contactItems.length > 0
        ...
        : 'No phone number, email, or contact form found on the page. Visitors who want to hire you literally cannot figure out how.'
    });
    ...
    if (pageData.hasJsonLd) {
      ...
      findings.push({ category: 'SEO', label: 'No structured data', pass: false, points: 0, maxPoints: 5, detail: 'No JSON-LD structured data found. Adding it can get you rich snippets in Google (star ratings, business hours, price ranges).' });
    }
